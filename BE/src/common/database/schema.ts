...
export const rooms = mysqlTable('rooms', {
	...
	createdBy: char('created_by', { length: 36 }).notNull().references(() => accounts.uuid),
	createdAt: datetime('created_at', { fsp: 6 }).default(sql`CURRENT_TIMESTAMP(6)`),
	updatedAt: datetime('updated_at', { fsp: 6 }).default(sql`CURRENT_TIMESTAMP(6)`),
})
...
export const roomParticipants = mysqlTable(
	...
	{
		uuid: char('uuid', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
		roomUuid: char('room_uuid', { length: 36 }).notNull().references(() => rooms.uuid),
		accountUuid: char('account_uuid', { length: 36 }).notNull().references(() => accounts.uuid),
		joinedAt: datetime('joined_at', { fsp: 6 }).default(sql`CURRENT_TIMESTAMP(6)`),
	},
	(table) => ({
		roomAccountIdx: uniqueIndex('uq_room_account').on(table.roomUuid, table.accountUuid),
	})
)
...
export const testCases = mysqlTable(
	...
	{
		uuid: char('uuid', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
		questionUuid: char('question_uuid', { length: 36 }).notNull().references(() => questions.uuid),
	...
	(table) => ({
		questionIndexIdx: uniqueIndex('uq_question_index').on(table.questionUuid, table.index),
	})
)
...
export const submissions = mysqlTable('submissions', {
	...
	status: mysqlEnum('status', ['PENDING', 'RUNNING', 'AC', 'WA', 'TLE', 'MLE', 'RE', 'CE', 'JUDGE_ERROR']).default('PENDING'),
	score: int('score'),
...
export const submissionDetails = mysqlTable('submission_details', {
	uuid: char('uuid', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
	submissionUuid: char('submission_uuid', { length: 36 }).notNull().references(() => submissions.uuid),
	testcaseUuid: char('testcase_uuid', { length: 36 }).notNull().references(() => testCases.uuid),
	index: int('index').notNull(),
	status: mysqlEnum('status', ['AC', 'WA', 'TLE', 'MLE', 'RE', 'SKIP']).notNull(),
...
export const submissionDetailsRelations = relations(submissionDetails, ({ one }) => ({
