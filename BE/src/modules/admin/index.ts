...
export const admin = new Elysia({ prefix: '/admin', tags: ['Admin'] })
	...
	.get('/users', adminService.getStudents, {
		detail: { summary: 'Get all students', description: 'Retrieve list of all users with role USER' },
		response: {
	...
	.get('/users/:userName', adminService.getStudentsByName, {
		detail: { summary: 'Search students by name', description: 'Search students by full name (partial match)' },
	...
	.patch('/users/:userId', adminService.updateUser, {
		detail: { summary: 'Update student info', description: 'Update student fullName and/or email. All fields are optional.' },
	...
	.post('/users/:userId/ban', adminService.banUser, {
		detail: { summary: 'Ban student', description: 'Ban a student account. Banned users cannot login.' },
		params: t.Object({ userId: t.String() }),
		response: {
	...
	.post('/users/:userId/unban', adminService.unbanUser, {
		detail: { summary: 'Unban student', description: 'Unban a previously banned student account.' },
		params: t.Object({ userId: t.String() }),
		response: {
			200: ApiResponseSchema(BanResponseSchema),
			400: ApiResponseSchema(t.Null()),
	...
	.get('/leaderboard/:roomId', adminService.getLeaderboard, {
		detail: { summary: 'Get room leaderboard', description: 'Get room info and list of students participating in the exam room.' },
	...
	.post('/room/add-student', adminService.addStudentToRoom, {
		detail: { summary: 'Add student to room', description: 'Admin manually adds a student to an exam room. Student must exist and not be an admin.' },
	...
	.get('/room/:roomId/participants', adminService.getRoomParticipants, {
		detail: { summary: 'Get room participants', description: 'Get list of all students who joined the exam room.' },
		params: t.Object({ roomId: t.String() }),
		response: {
	...
	.get('/room/:roomId/participants/search/:studentName', adminService.searchRoomParticipants, {
		detail: { summary: 'Search room participants', description: 'Search students in a room by name (partial match).' },
	...
	.delete('/room/:roomId/participants/:studentId', adminService.removeStudentFromRoom, {
		detail: { summary: 'Remove student from room', description: 'Remove a student from an exam room.' },
