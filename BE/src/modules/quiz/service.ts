...
export const quizService = {
		...
		for (const q of data.questions) {
			...
			if (correctCount === 0) {
				...
				return wrapResponse(
					...
					400,
					'',
					`Question "${q.content}" must have at least one correct answer.`
				)
			}
		}
	...
	getQuiz: async ({ params, set, user }: any) => {
		...
		// Check if user is admin to include isCorrect
		const isAdmin = user?.isAdmin === true

		// Map to DTO, include isCorrect only for admin
		const response = {
			uuid: quiz.uuid,
			title: quiz.title,
	...
	submitQuiz: async ({ params, body, user, set }: any) => {
		...
		for (const question of quiz.questions) {
			...
			const userAnswer = userAnswers.find(
				ua => ua.questionUuid === question.uuid
			)
			const selectedIds = new Set(
				userAnswer ? userAnswer.selectedAnswerUuids : []
			)
			...
			if (correctAnswers.length > 0) {
				const questionScore =
					(correctSelectedCount / correctAnswers.length) * question.points
				totalScore += questionScore
			}
		}

		// Save Submission
		await quizRepository.createSubmission(
			quiz.uuid,
			userId,
			totalScore,
			totalPossiblePoints
	...
	importQuizQuestions: async ({ body, set }: any) => {
		...
		if (!data.quizUuid && !data.title) {
			...
			return wrapResponse(
				...
				400,
				'',
				'Either quizUuid (to add to existing quiz) or title (to create new quiz) is required'
			)
		}
		...
		for (let i = 0; i < data.questions.length; i++) {
			...
			if (correctCount === 0) {
				errors.push({
					index: i,
					reason: 'Must have at least one correct answer'
				})
				skipped++
			}
		}
		...
		if (skipped === data.questions.length) {
			...
			return wrapResponse(
				...
				400,
				'',
				'All questions are invalid. Each question must have at least one correct answer.'
			)
		}
		...
		try {
			...
			return wrapResponse(
				response,
				201,
				`Successfully imported ${imported} question(s)`
			)
		} catch (error: any) {
	...
	copyQuestions: async ({ params, body, set }: any) => {
		...
		try {
			const result = await quizRepository.copyQuestionsFromQuiz(
				targetQuizUuid,
				data.sourceQuizUuid,
				data.questionUuids
			)
			...
			return wrapResponse(
				response,
				201,
				`Successfully copied ${result.copied} question(s)`
			)
		} catch (error: any) {
