...
export default function QuestionTestCasesPage() {
	...
	const generateId = useCallback(() => {
		return `tc_${Math.random().toString(36).substring(2, 9)}`
	}, [])
	...
	const handleSave = () => {
		if (editingTestCase) {
			setTestCases(
				testCases.map(tc =>
					tc.testcaseId === editingTestCase.testcaseId
						? { ...tc, ...formData }
						: tc
				)
			)
		...
		<div className="container mx-auto p-6">
					...
					<div className="space-y-4">
						...
						{testCases.length === 0 && (
							<div className="text-center py-12 text-muted-foreground">
								Chưa có test case nào. Nhấn &quot;Thêm test case&quot; để tạo
								mới.
							</div>
						)}
					</div>
			...
			<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
				<DialogContent className="max-w-lg">
					<DialogHeader>
						<DialogTitle>
							{editingTestCase ? 'Sửa test case' : 'Thêm test case mới'}
						</DialogTitle>
						<DialogDescription>
							Điền thông tin test case cho câu hỏi
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
						...
						<div className="flex items-center space-x-2">
							...
							<Label htmlFor="isHidden">
								Ẩn test case (không hiển thị cho học sinh)
							</Label>
						</div>
					</div>
					<DialogFooter>
						...
						<Button onClick={handleSave}>
							{editingTestCase ? 'Cập nhật' : 'Tạo test case'}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
